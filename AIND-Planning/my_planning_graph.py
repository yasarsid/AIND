...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            precond = action_node.precond_s_nodes()
            expected_s_nodes = self.s_levels[level]
            if precond.issubset(expected_s_nodes):
                list.append(action_node)
        self.a_levels.append(list)
    ...
    def add_literal_level(self, level):
        ...
        #  Since we are working with sets, they
        ...
        s_nodes = set()
        for a_nodes in previous_action_nodes:
            s_nodes = s_nodes.union(a_nodes.effnodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        inconsistent_effects = is_intersection_present(node_a1.action.effect_add, node_a2.action.effect_rem) or \
                               is_intersection_present(node_a2.action.effect_add, node_a1.action.effect_rem)
        return inconsistent_effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        interference_mutex = is_intersection_present(node_a1.action.effect_add, node_a2.action.precond_pos) or \
                             is_intersection_present(node_a1.action.effect_rem, node_a2.action.precond_neg) or \
                             is_intersection_present(node_a2.action.effect_add, node_a1.action.precond_pos) or \
                             is_intersection_present(node_a2.action.effect_rem, node_a1.action.precond_neg)
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if state.is_pos and state.symbol == goal and goal not in explored:
...
def is_intersection_present(list_one: list, list_two: list):
    return len(set(list_one).intersection(list_two)) > 0
